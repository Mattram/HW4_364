...
# TODOdone 364: Check out the included file giphy_api_key.py and follow the instructions in TODOs there before proceeding to view functions.

# TODOdone 364: All templates you need are provided and should not be edited. However, you will need to inspect the templates that exist in order to make sure you send them the right data!
...
# TODOdone 364: Set up association Table between search terms and GIFs (you can call it anything you want, we suggest 'tags' or 'search_gifs').
tags = db.Table('tags', db.Column('user_id', db.Integer, db.ForeignKey('gifs.id')), db.Column('searches_ID',db.Integer, db.ForeignKey('searches.id')))


# TODOdone 364: Set up association Table between GIFs and collections prepared by user (you can call it anything you want. We suggest: user_collection)
user_collection = db.Table('user_collection', db.Column('user_id', db.Integer, db.ForeignKey('gifs.id')), db.Column('gifCollection_id',db.Integer, db.ForeignKey('gifCollection.id')))
...
class User(UserMixin, db.Model):
    ...
    #TODOdone 364: In order to complete a relationship with a table that is detailed below (a one-to-many relationship for users and gif collections), you'll need to add a field to this User model. (Check out the TODOs for models below for more!)
    # Remember, the best way to do so is to add the field, save your code, and then create and run a migration!
    gifCollection = db.relationship('PersonalGifCollection', backref='User')
...
# TODOdone 364: Read through all the models tasks before beginning them so you have an understanding of what the database structure should be like. Consider thinking about it as a whole and drawing it out before you write this code.
...
class Gif(db.Model):
    ...
    embedURL = db.Column(db.String(256))
     # Replace with code
    # TODOdone 364: Add code for the Gif model such that it has the following fields:
    # id (Integer, primary key)
    # title (String up to 128 characters)
    # embedURL (String up to 256 characters)

    # TODOdone 364: Define a __repr__ method for the Gif model that shows the title and the URL of the gif
    def __repr__(self):
        return 'Title: %s (URL: %s)' % (self.title, self.embedURL)

# Model to store a personal gif collection
class PersonalGifCollection(db.Model):
    __tablename__ = "gifCollection"
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(255))
    user_id = db.Column(db.Integer, db.ForeignKey("users.id"))
    gifs = db.relationship('Gif', secondary=user_collection, backref=db.backref('gifCollection', lazy='dynamic'),lazy='dynamic')
    # TODOdone 364: Add code for the PersonalGifCollection model such that it has the following fields:
    # id (Integer, primary key)
...
class SearchTerm(db.Model):
    ...
    # TODOdone 364: Add code for the SearchTerm model such that it has the following fields:
    # id (Integer, primary key)
    ...
    # TODOdone 364: Define a __repr__ method for this model class that returns the term string
...
# TODOdone 364: The following forms for searching for gifs and creating collections are provided and should not be edited. You SHOULD examine them so you understand what data they pass along and can investigate as you build your view functions in TODOs below.
...
def get_gifs_from_giphy(search_string):
    ...
    # TODOdone 364: This function should make a request to the Giphy API using the input search_string, and your api_key (imported at the top of this file)
...
def get_or_create_gif(title, url):
    ...
    return gif # Replace with code
    # TODOdone 364: This function should get or create a Gif instance. Determining whether the gif already exists in the database should be based on the gif's title.
...
def get_or_create_search_term(term):
    ...
    if not search:
        ...
        for g in gifs:
            search.gifs.append(get_or_create_gif(g.title, g.embed_url)
        db.session.add(search)
        db.session.commit()
    return search
    # TODOdone 364: This function should return the search term instance if it already exists.
...
def get_or_create_collection(name, current_user, gif_list=[]):
    ...
    gifcol = PersonalGifCollection.query.filter_by(name = name, user_id=current_user).first()
    if not gifcol:
        gifcol = PersonalGifCollection(name = name, user_id = current_user)
        for g in gif_list:
            gifcol.gifs.append(get_or_create_gif(g.title, g.embed_URL))
    return gifcol

    # TODOdone 364: This function should get or create a personal gif collection. Uniqueness of the gif collection should be determined by the name of the collection and the id of the logged in user.
...
def index():
    ...
    # TODOdone 364: Edit this view function, which has a provided return statement, so that the GifSearchForm can be rendered.
...
def search_terms():
    terms = SearchTerm.query.all()
     # Replace with code
    # TODOdone 364: Edit this view function so it renders search_terms.html.
...
@login_required
def collections():
    personalCollection = PersonalGifCollection.query.all()
    return render_template('collections.html', collections=personalCollection)
    # TODOdone 364: This view function should render the collections.html template so that only the current user's personal gif collection links will render in that template. Make sure to examine the template so that you send it the correct data!
